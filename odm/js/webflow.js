...
Webflow.define('forms', function ($, _) {
  ...
  function build(i, el) {
	...
	// ignore - we dont use webflows backend
    disconnected();
  }
  ...
  function submitWebflow(data) {
	...
	// submit only if it is a real form...
	// cmdform gets handled by javascript
	if(payload.name != "cmdform") {
		// Read forma Name
		// NOTE: If this site is exported, the HTML tag must retain the data-wf-site attribute for forms to work
		if (!siteId) { afterSubmit(data); return; }
		
		var cLoc = window.location.pathname;
		var cDir = cLoc.substring(0, cLoc.lastIndexOf('/'));
		var url = window.location.protocol + '//' + window.location.hostname + '' + cDir + '/ajax/connector.php?cmd=formsubmit&form=' + payload.name;
		...
		$.ajax({
		  ...
		  if (resp.result === "success") {
			data.success = true;
			data.hideonsuccess = true;
		  } else {
			data.errormsg = resp.msg;
			if(resp.err_field)
				data.errfield = resp.err_field;
		  }
		  afterSubmit(data);
		}).fail(function () {
		  afterSubmit(data);
		...
		data.success = true;
		var devId = payload.fields.devid;
		switch(payload.fields.dropdown) {
			case "locate": sendPushNotification(devId, 'Type=location:Command:GetLocation'); hideControls(); break;
			case "locategps": sendPushNotification(devId, 'Type=location:Command:GetLocationGPS'); hideControls(); break;
			case "lock": sendPushNotification(devId, 'Type=lock:Command:Lock'); break;
			case "lockpassword": sendPushNotification(devId, "Type=lock:Command:LockPass:"+payload.fields.value); break;
			case "rearcam": sendPushNotification(devId, 'Type=photo:Command:RearPhoto'); break;
			case "frontcam": sendPushNotification(devId, 'Type=photo:Command:FrontPhoto'); break;
			case "ring": sendPushNotification(devId, 'Type=ring:Command:StartRing'); break;
			case "ringoff": sendPushNotification(devId, 'Type=ring:Command:StopRing'); break;
			case "sms":
				if(payload.fields.value.match(/^[0-9+]+$/) == null) { // incorrect number!
					data.success = false;
					break;
				}
				if(payload.fields.value2 == "")
					sendPushNotification(devId, "Type=sms:Command:SMS:"+payload.fields.value);
				else
					sendPushNotification(devId, "Type=sms:Command:CustomSMS:"+payload.fields.value+":"+payload.fields.value2);
				break;
			case "wipe": 
				if (confirm("Das löscht ALLE Daten auf Ihrem Gerät!! Sind Sie sicher?")) {
					sendPushNotification(devId, "Type=wipe:Command:Wipe");
				}	
				break;
			...
			case "delete": 
				if (confirm("Das löscht Ihr Gerät aus dem Sprinternet Device Manager! Sind Sie sicher?")) {
					...
					window.location.href = "delete.php?id="+devId;
				}
				break;			
			default: data.success = false; break;
		}
		afterSubmit(data);
	}
  }
  ...
  function afterSubmit(data) {
	...
	// reset all failed fields first
	form.find("input[class*='formerror']").each(function() {
		$(this).removeClass("formerror");
	});
	
	if(!success && data.errormsg) {
		data.fail.html("<p>Fehler: " + data.errormsg + "</p>");
		if(data.errfield) {
			form.find("input[name*='"+data.errfield+"']").first().addClass("formerror");
		}
	}
